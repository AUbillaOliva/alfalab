...
router.post(
  ...
  async (req, res) => {
    ...
    if (orderList) {
      deliveryFields.orderList = orderList;
    }
    if (client) {
      deliveryFields.client = client;
    }
    if (zone) {
      deliveryFields.zone = zone;
    }
    if (delivered_by) {
      deliveryFields.delivered_by = delivered_by;
    }
    if (orders_number) {
      deliveryFields.orders_number = orders_number;
    }
    if (created_at) {
      deliveryFields.created_at = created_at;
    }
    if (delivered_date) {
      deliveryFields.delivered_date = delivered_date;
    }
    if (total) {
      deliveryFields.total = total;
    }
    ...
    try {
      let delivery = await Delivered.findOne({
