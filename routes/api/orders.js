...
router.get("/", async (req, res) => {
  try {
    ...
    if (!data) {
      return res.status(200).json({ msg: "There is not orders" });
    } else {
...
router.post(
  ...
  async (req, res) => {
    ...
    const ordersFields = {};
    if (orderList) {
      ordersFields.orderList = orderList;
    }
    if (client) {
      ordersFields.client = client;
    }
    if (zone) {
      ordersFields.zone = zone;
    }
    if (delivered_by) {
      ordersFields.delivered_by = delivered_by;
    }
    if (created_at) {
      ordersFields.created_at = created_at;
    }
    if (delivered_date) {
      ordersFields.delivered_date = delivered_date;
    }
    if (total) {
      ordersFields.total = total;
    }
    if (status != null || status != undefined) {
      ordersFields.status = status;
    }
    ...
    try {
      ...
      if (orders) {
        orders = await Orders.findOneAndUpdate(
          { orders_number: req.params.number },
          {
            $set: ordersFields,
            new: false,
          }
...
router.delete("/:number", auth, async (req, res) => {
  try {
    ...
    if (orders) {
      ...
      return res.status(200).send(`Orders ${orders._id} deleted`);
    } else {
      return res.status(404).send(httpError.NotFound("Order not found"));
    }
  } catch (error) {
    console.error(error.message);
    return res.status(500).send(httpError.InternalServerError("Server error"));
